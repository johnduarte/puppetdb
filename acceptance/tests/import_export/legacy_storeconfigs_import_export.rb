test_name "storeconfigs export and import" do
  ...
  master_pup_ver = on(master, "facter puppetversion").stdout.chomp
  skip_test "storeconfigs not supported in puppet >= 4.0" unless version_is_less(master_pup_ver, '4.0.0')
  ...
  step "Verify imported catalogs" do
    hosts.each do |host|
      result = on database, %Q|curl -G http://#{database}:8080/v4/catalogs/#{host.node_name}|
