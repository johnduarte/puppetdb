...
test_name "validate that nodes are deactivated and deleted based on ttl settings" do
  ...
  step "Verify that the nodes were deactivated but not deleted" do
    ...
    agents.each do |agent|
      result = on database, %Q|curl -G http://#{database}:8080/v3/nodes/#{agent.node_name}|
  ...
  step "Verify that all associated data was deleted" do
    result = on database, "curl -G http://#{database}:8080/v3/facts/operatingsystem"
    ...
    result = on database, %Q|curl -G http://#{database}:8080/v3/resources --data 'query=["=","exported",false]'|
    ...
    agents.each do |agent|
      result = on database, %Q|curl -G http://#{database}:8080/v3/reports --data 'query=["=","certname","#{agent.node_name}"]'|
