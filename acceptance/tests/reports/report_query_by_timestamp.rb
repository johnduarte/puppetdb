...
test_name "basic validation of puppet report query by timestamp" do
  ...
  agents.each do |agent|
    ...
    result = on database, %Q|curl -G http://#{database}:8080/v3/events --data 'query=["and",%20["=",%20"certname",%20"#{agent.node_name}"],%20[">",%20"timestamp",%20"#{start_times[agent.node_name]}"]]'|
  ...
  agents.each do |agent|
    ...
    # Query for all of the events after the start time
    result = on database, %Q|curl -G http://#{database}:8080/v3/events --data 'query=["and",%20["=",%20"certname",%20"#{agent.node_name}"],%20[">",%20"timestamp",%20"#{start_time}"]]'|
    ...
    result = on database, %Q|curl -G http://#{database}:8080/v3/events --data 'query=["and",%20["=",%20"certname",%20"#{agent.node_name}"],%20["and",%20[">",%20"timestamp",%20"#{start_time}"],%20["<",%20"timestamp",%20"#{end_time}"]]]'|
