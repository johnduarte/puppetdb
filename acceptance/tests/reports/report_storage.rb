...
test_name "basic validation of puppet report submission" do
  ...
  agents.each do |agent|
    ...
    result = on database, %Q|curl -G http://#{database}:8080/v3/reports --data 'query=["=",%20"certname",%20"#{agent.node_name}"]'|
    ...
    result = on database, %Q|curl -G 'http://#{database}:8080/v3/events' --data 'query=["=",%20"report",%20"#{report["hash"]}"]'|
